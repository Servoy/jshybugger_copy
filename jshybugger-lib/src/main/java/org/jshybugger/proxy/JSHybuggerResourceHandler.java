...
class JSHybuggerResourceHandler implements HttpHandler {
	...
	public JSHybuggerResourceHandler(ScriptSourceProvider scriptSourceProvider, DebugServer debugServer) {
	...
	private JSDInterface getBrowserInterface(HttpRequest req, HttpResponse resp, boolean createSession) {
		...
		if (debugSession == null) {
			if  (createSession) {
				...
				{
					public String loadScriptResourceById(String scriptUri,
							boolean encode) throws IOException {
						return scriptSourceProvider.loadScriptResourceById(
								scriptUri, encode);
					}

					@Override
					public String setScriptSource(JSONObject message)
							throws JSONException {
						String scriptUri = message.getJSONObject("params")
								.getString("scriptId");
						String scriptSource = message.getJSONObject("params")
								.getString("scriptSource");
						return scriptSourceProvider.setScriptSource(scriptUri,
								scriptSource);
					}
				};
